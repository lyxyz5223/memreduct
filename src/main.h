...
#include <stdlib.h>
#include <string.h>
bool AlreadyHook = false;
bool is_MIDDLECLICK = false;
POINT mPoint;//托盘，鼠标钩子
POINT mpoint;//比较，钩子，鼠标pos
HWND windowHWND;
HHOOK hhook;
//Windows Hook
LRESULT CALLBACK HookProc (int code, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
	if (code < 0)
	...
	else
	{
...
#ifdef _DEBUG
				wchar_t ds[256] = L"";
				wsprintf (ds, L"(钩子)鼠标pos：%d,%d\n", mpoint.x, mpoint.y);
				OutputDebugString (ds);
#endif

				if (mpoint.x == mPoint.x && mpoint.y == mPoint.y)
				{
					//BroadcastSystemMessage()
					PostMessage (windowHWND, RM_TRAYICON, WM_MBUTTONDOWN, WM_MBUTTONDOWN);
#ifdef _DEBUG
					MessageBox (0, L"MiddleClickDOWN！！", L"", 0);
#endif
		...
		if (wParam == WM_MBUTTONUP)
		{
			if (GetCursorPos (&mpoint))
			{
				...
				{
					//BroadcastSystemMessage()
					PostMessage (windowHWND, RM_TRAYICON, WM_MBUTTONUP, WM_MBUTTONUP);
#ifdef _DEBUG
					MessageBox (0, L"MiddleClickUP！！", L"", 0);
#endif
